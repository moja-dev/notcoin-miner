...
// @grant       GM_setValue
// @version     1.0
// @author      moja web : (moja-dev.ir)
// @homepageURL https://github.com/moja-dev/notcoin-miner
// ==/UserScript==
...
const init = async () => {
  ...
  await waitForElm('.reply-markup-button[href="https://t.me/notcoin_bot/click"]').then(() => {
    const button = document.querySelector('.reply-markup-button[href="https://t.me/notcoin_bot/click"]');
    button.click();
...
const openpopup = async () => {
  console.info('%c INIT: start initialization', 'color: #64b5f6');
  await waitForElm('.popup-buttons button').then(() => {
    const button = document.querySelector('.popup-buttons button');
    button.click();
    console.info('%c INIT: initializated', 'color: #64b5f6');
  });
}
